...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if doc.file_size > MAX_FILE_SIZE_MB * 1024 * 1024:
        await update.message.reply_text(
            f"Fayl hajmi {MAX_FILE_SIZE_MB}MB dan katta. Kichikroq fayl yuboring.\n"
            f"Файл больше {MAX_FILE_SIZE_MB}МБ. Отправьте файл меньшего размера."
        )
        return
    ...
    try:
        ...
        # PDF ni Claude ga to'g'ridan-to'g'ri yuborish
        if "pdf" in mime:
        ...
        # Matnli fayllar (.txt, .csv, .py va h.k.)
            ...
            await update.message.reply_text(
                "Bu fayl turini qo'llab-quvvatlamayman. PDF, TXT, CSV fayllarini yuboring.\n"
                "Этот тип файла не поддерживается. Отправьте PDF, TXT или CSV."
            )
